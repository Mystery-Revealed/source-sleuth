...
// the PDF → End Session (deletes everything from memory). Source Sleuth is
// solo: everyone works the same case file and starts as soon as they're
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Source Sleuth · everyone works the same case file · TEKS 7.20A, 7.20D, 7.20E, 7.20F</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Source Sleuth · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student opens their case file right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-accuracy">
          <div className="trail-stat wide">
            <div className="trail-stat-name"><span aria-hidden="true">🔍</span> Your Case File — the whole class</div>
      ...
      <footer className="app-footer">7th Grade Texas History · Source Sleuth · TEKS 7.20A, 7.20D, 7.20E, 7.20F</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [90, 62, 30]; // case-file brown

  doc.setFontSize(16);
  doc.text('Source Sleuth — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    body: [['Your Case File', String(cls.count), cls.count ? `${cls.average}%` : '—']],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Source Sleuth · TEKS 7.20A, 7.20D, 7.20E, 7.20F', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`source_sleuth_${roster.joinCode}_${date}.pdf`);
}
