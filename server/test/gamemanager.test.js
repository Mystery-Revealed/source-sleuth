...
// and inspects the emit instructions it returns. No sockets involved. Source
// Sleuth is solo with ONE class-wide role (a single side, no rival), so
...
import game, { PHASES } from '../src/games/sourceSleuth.js';
...
test('the default game is Source Sleuth', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'sourceSleuth');
});
...
test('solo student works the case and completes with 100% accuracy → Master Sleuth', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  ...
  assert.equal(begin[0].payload.side, SIDE, 'the one side is the historian');
  assert.equal(begin[0].payload.mode, 'solo');
  assert.equal(begin[0].payload.chapterCount, 6, 'six sources');
  assert.equal(begin[0].payload.rivalMeters, null, 'no rival');
  assert.deepEqual(begin[0].payload.meters, { evidence: 50, credibility: 50, case: 50 });
  ...
  assert.equal(end[0].payload.you.ending.key, 'top', 'a perfect run is Master Sleuth');
...
test('all-wrong run plays all 12 actions to the end (no early-fail) → Reopened for Review, 0%', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('class accuracy is one class-wide group (the historian)', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
